...
use crate::handler;
use crate::repository::{MeuexemploRepository, MeuexemploRepositoryImpl};
use crate::service::{MeuexemploService, MeuexemploServiceImpl};

pub struct App {
    read_pool: Option<Arc<PgPool>>,
    write_pool: Option<Arc<PgPool>>,
    service: Option<Arc<dyn MeuexemploService>>,
}
...
impl App {
    ...
    pub async fn start(&mut self) {
        ...
        let repo: Arc<dyn MeuexemploRepository> = Arc::new(MeuexemploRepositoryImpl::new(read_pool, write_pool));
        let service: Arc<dyn MeuexemploService> = Arc::new(MeuexemploServiceImpl::new(repo));
    ...
    pub async fn run(&mut self, port: &str) -> anyhow::Result<()> {
        let service = self.service.clone().expect("Call start() first");
        let api = handler::routes(service);
